...
public class JobManager {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		createClusterAndRunHiveJob();
	}

	private static void createBucketAndPutNecessaryObjects() {
		try {
			s3client.createBucket(CreateBucketRequest.builder().bucket("emr-hive-bucket-1").build());
		} catch (BucketAlreadyExistsException e) {
			e.printStackTrace();
		}
		s3client.putObject(PutObjectRequest.builder().bucket("emr-hive-bucket-1").key("names.txt").build(),
				RequestBody.fromFile(new File("names.txt")));
		s3client.putObject(PutObjectRequest.builder().bucket("emr-hive-bucket-1").key("names.sql").build(),
				RequestBody.fromFile(new File("names.sql")));
	}

	private static void createClusterAndRunHiveJob() throws InterruptedException {
		Application hadoop = Application.builder().name("Hadoop").build();
		Application hive = Application.builder().name("Hive").build();

		StepConfig hiveJobStepConfig = StepConfig.builder().name("Simple Hive Job")
				.actionOnFailure(ActionOnFailure.TERMINATE_CLUSTER)
				.hadoopJarStep(HadoopJarStepConfig.builder().jar("command-runner.jar")
						.args("hive-script", "--run-hive-script", "--args", "-f", "s3://emr-hive-bucket-1/names.sql")
						.build())
				.build();

		RunJobFlowRequest request = RunJobFlowRequest.builder().name("My EMR Cluster").releaseLabel("emr-6.12.0")
				.applications(hadoop, hive).logUri("s3://emr-hive-bucket-1/logs").serviceRole("EMR_DefaultRole")
